...
void cache_public_key(PKT_public_key *pk)
{
    ...
    if (is_ELGAMAL(pk->pubkey_algo) || pk->pubkey_algo == PUBKEY_ALGO_DSA || pk->pubkey_algo == PUBKEY_ALGO_ECDSA || pk->pubkey_algo == PUBKEY_ALGO_EDDSA || pk->pubkey_algo == PUBKEY_ALGO_ECDH || is_RSA(pk->pubkey_algo))
    {
...
get_primary_uid(KBNODE keyblock, size_t *uidlen)
{
    ...
    {
        if (k->pkt->pkttype == PKT_USER_ID && !k->pkt->pkt.user_id->attrib_data && k->pkt->pkt.user_id->flags.primary)
        {
...
cache_user_id(KBNODE keyblock)
{
    ...
    for (k = keyblock; k; k = k->next)
    {
        if (k->pkt->pkttype == PKT_PUBLIC_KEY || k->pkt->pkttype == PKT_PUBLIC_SUBKEY)
        {
            ...
            pubkey_t *r_keys)
{
   ...
   * this list.  */
    ...
    switch (use)
    {
    case PUBKEY_USAGE_ENC:
        use_str = "encrypt";
        break;
    case PUBKEY_USAGE_SIG:
        use_str = "sign";
        break;
    case PUBKEY_USAGE_CERT:
        use_str = "cetify";
        break;
    case PUBKEY_USAGE_AUTH:
        use_str = "authentication";
        break;
    default:
        log_bug("%s: Bad value for USE (%d)\n", __func__, use);
    }
    ...
    if (warn_possibly_ambiguous && !(desc.mode == KEYDB_SEARCH_MODE_LONG_KID || desc.mode == KEYDB_SEARCH_MODE_FPR16 || desc.mode == KEYDB_SEARCH_MODE_FPR20 || desc.mode == KEYDB_SEARCH_MODE_FPR))
    {
...
pk_from_block(PKT_public_key *pk, kbnode_t keyblock, kbnode_t found_key)
{
    ...
    log_assert(a->pkt->pkttype == PKT_PUBLIC_KEY || a->pkt->pkttype == PKT_PUBLIC_SUBKEY);
...
int get_pubkey_fast(PKT_public_key *pk, u32 *keyid)
{
...
#if MAX_PK_CACHE_ENTRIES
    {
        ...
        for (ce = pk_cache; ce; ce = ce->next)
        {
            if (ce->keyid[0] == keyid[0] && ce->keyid[1] == keyid[1]
                ...
                && ce->pk->keyid[0] == ce->pk->main_keyid[0] && ce->pk->keyid[1] == ce->pk->main_keyid[1])
            {
                if (pk)
                    copy_public_key(pk, ce->pk);
                return 0;
    ...
    log_assert(keyblock && keyblock->pkt && keyblock->pkt->pkttype == PKT_PUBLIC_KEY);
    ...
    else
    {
	 ...
	 CTX->ITEMS.  */
        ...
        for (n = 0, r = namelist; r; r = r->next, n++)
        {
            ...
            if (!include_unusable && ctx->items[n].mode != KEYDB_SEARCH_MODE_SHORT_KID && ctx->items[n].mode != KEYDB_SEARCH_MODE_LONG_KID && ctx->items[n].mode != KEYDB_SEARCH_MODE_FPR16 && ctx->items[n].mode != KEYDB_SEARCH_MODE_FPR20 && ctx->items[n].mode != KEYDB_SEARCH_MODE_FPR)
            {
 ...
 * returned.  In particular, GPG_ERR_NO_PUBKEY or GPG_ERR_NO_SECKEY
 * (if want_secret is set) is returned if the key is not found.  */
int get_pubkey_byname(ctrl_t ctrl, GETKEY_CTX *retctx, PKT_public_key *pk,
...
{
    return (!key->has_expired && !key->flags.revoked && key->flags.valid && !key->flags.disabled);
}
           ...
           struct pubkey_cmp_cookie *new, KBNODE new_keyblock)
{
         ...
         n; n = find_next_kbnode(n, PKT_USER_ID))
    {
        ...
        if (old->validity == new->validity && uid_is_ok(&new->key, uid) && old->creation_time < new->creation_time)
                           ...
                           const byte *fprint, size_t fprint_len, int lock)
{
    ...
    log_assert(keyblock->pkt->pkttype == PKT_PUBLIC_KEY || keyblock->pkt->pkttype == PKT_PUBLIC_SUBKEY);
                    ...
                    struct revoke_info *rinfo)
{
   ...
   * it.  */
    for (k = keyblock;
         k && k->pkt->pkttype != PKT_USER_ID && k->pkt->pkttype != PKT_ATTRIBUTE && k->pkt->pkttype != PKT_PUBLIC_SUBKEY;
         k = k->next)
    {
	   ...
	   * UIDNODE follows it.  */
        ...
        else if (k->pkt->pkttype == PKT_SIGNATURE && uidnode)
        {
            ...
            if (sig->keyid[0] == kid[0] && sig->keyid[1] == kid[1])
            {
                if (check_key_signature(ctrl, keyblock, k, NULL))
                    ; /* signature did not verify */
                else if ((IS_UID_SIG(sig) || IS_UID_REV(sig)) && sig->timestamp >= sigdate)
                {
    ...
    if (!pk->flags.valid)
    {
             ...
             k = k->next)
        {
		   ...
		   * should also remove the ultimate trust flag.  */
                    if (get_pubkey_fast(ultimate_pk, sig->keyid) == 0 && check_key_signature2(ctrl, keyblock, k, ultimate_pk, NULL, NULL, NULL, NULL) == 0 && get_ownertrust(ctrl, ultimate_pk) == TRUST_ULTIMATE)
                    {
...
#define USAGE_MASK (PUBKEY_USAGE_SIG | PUBKEY_USAGE_ENC | PUBKEY_USAGE_CERT)
   ...
   * not understand signatures made by a signing subkey.  PGP 8 does. */
    req_prim = ((req_usage & PUBKEY_USAGE_CERT) || ((PGP6 || PGP7) && (req_usage & PUBKEY_USAGE_SIG)));
    ...
    if (want_exact)
    {
        for (k = keyblock; k; k = k->next)
        {
            if ((k->flag & 1))
            {
                log_assert(k->pkt->pkttype == PKT_PUBLIC_KEY || k->pkt->pkttype == PKT_PUBLIC_SUBKEY);
                foundk = k;
...
print_status_key_considered(kbnode_t keyblock, unsigned int flags)
{
    ...
    for (node = keyblock; node; node = node->next)
        if (node->pkt->pkttype == PKT_PUBLIC_KEY || node->pkt->pkttype == PKT_SECRET_KEY)
            break;
    if (!node)
    {
...
enum_secret_keys(ctrl_t ctrl, void **context, PKT_public_key *sk)
{
    ...
    for (;;)
    {
        ...
        {
            if (c->node->pkt->pkttype == PKT_PUBLIC_KEY || c->node->pkt->pkttype == PKT_PUBLIC_SUBKEY)
            {
                           ...
                           const byte *fpr_card, size_t fpr_len)
{
    ...
    if (!fpr_card || (def_secret_key && def_secret_key[strlen(def_secret_key) - 1] == '!'))
...
get_user_id_byfpr(ctrl_t ctrl, const byte *fpr, size_t *rn)
{
    ...
    } while (++pass < 2 && !get_pubkey_byfprint(ctrl, NULL, NULL, fpr, MAX_FINGERPRINT_LEN));
    p = xstrdup(user_id_not_found_utf8());
    *rn = strlen(p);
    return p;
}
...
int parse_auto_key_locate(const char *options_arg)
{
    char *tok;
    ...
    while ((tok = optsep(&options)))
    {
        ...
        {
            /* Check for duplicates */
            if (check->type == akl->type && (akl->type != AKL_SPEC || (akl->type == AKL_SPEC && strcmp(check->spec->uri, akl->spec->uri) == 0)))
            {
                dupe = 1;
                free_akl(akl);
                break;
...
int have_secret_key_with_kid(u32 *keyid)
{
    ...
    while (!result)
    {
        ...
        for (node = keyblock; node; node = node->next)
        {
            ...
            if ((node->flag & 1))
            {
                log_assert(node->pkt->pkttype == PKT_PUBLIC_KEY || node->pkt->pkttype == PKT_PUBLIC_SUBKEY);
